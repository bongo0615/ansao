...
export function KhungChat({
  ...
  async function gui(noiDung: string) {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          laSoId,
          laSo: laSoId ? undefined : laSo && {
            hoTen: laSo.hoTen || "Đương số", gioiTinh: laSo.gioiTinh,
            namSinh: laSo.namSinh, thangSinh: laSo.thangSinh, ngaySinh: laSo.ngaySinh,
            gioSinh: laSo.gioSinh, phutSinh: laSo.phutSinh, noiSinh: laSo.noiSinh,
            namXem: laSo.namXem ?? null, thangXem: laSo.thangXem ?? null,
  ...
  return (
    <div className="glass flex h-full min-h-0 flex-col rounded-2xl">
      ...
      <div className="flex items-center gap-3 border-b border-line px-5 py-4">
        ...
        <div className="min-w-0">
          ...
          <p className="truncate text-xs text-ink-faint">
            {dangChay ? "đang luận giải…" : `đang xem lá số ${tenDuongSo}`}
          </p>
        </div>
      </div>
      ...
      <div ref={cuonRef} className="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-5">
        {tin.length === 0 && (
          <div className="py-4">
            <p className="text-sm leading-relaxed text-ink-dim">
              Tôi đã xem qua lá số của <strong className="text-ink">{tenDuongSo}</strong>.
        ...
        {tin.map((m, i) => (
          <div key={i} className={m.vai === "user" ? "flex justify-end" : ""}>
            {m.vai === "user" ? (
              <p className="max-w-[85%] whitespace-pre-wrap rounded-2xl rounded-br-md bg-hanh-kim/20
                            px-4 py-2.5 text-sm leading-relaxed">{m.noiDung}</p>
            ) : (
              <div className="max-w-none text-[15px] text-ink-dim">
                {m.noiDung
                  ...
                  : <span className="inline-flex gap-1.5 py-1" aria-label="Đang soạn">
                      {[0, 1, 2].map((d) => (
                        <span key={d} className="h-1.5 w-1.5 rounded-full bg-ink-faint"
      ...
      {/* Ô nhập */}
      <form
        onSubmit={(e) => { e.preventDefault(); gui(nhap); }}
        className="border-t border-line p-3"
      >
        <div className="flex items-end gap-2 rounded-xl border border-line bg-white/[0.04] px-3 py-2
                        transition focus-within:border-cyan/40">
          <textarea
            value={nhap}
            onChange={(e) => setNhap(e.target.value)}
            onKeyDown={(e) => {
              if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); gui(nhap); }
            }}
            rows={1}
